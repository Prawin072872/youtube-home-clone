...
export function Sidebar() {
    ...
    scrollbar-hidden pb-4 flex flex-col ml-1 ${isLargeOpen ? "lg:hidden" : "lg:flex"}`}>
    ...
    pb-4 flex-col gap-2 px-2 ${isLargeOpen ? "lg:flex" : "lg:hidden"} ${isSmallOpen ? "flex z-[999] bg-white max-h-screen" : "hidden"}`}>
...
function SmallSidebarItem({ IconOrImgUrl, title, url }: SmallSidebarItemProps) {
    return <a href={url} className={twMerge(buttonStyles(
        { variant: "ghost" }), "py-4 px-1 flex flex-col items-center rounded-lg gap-1")}>
