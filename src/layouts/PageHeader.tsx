...
import Youtube from "../assets/youtubetest.jpg"
        ...
        <form className={`gap-4 flex-grow justify-center  ${showFullWidthSearch ? "flex" : "hidden md:flex"}`}>
            ...
            <div className="flex flex-grow max-w-[600px]">
                <input type="search" placeholder="Search"
                    className="rounded-l-full border focus:border-blue-500 outline-none border-secondary-border shadow-inner shadow-secondary py-1 px-4 text-lg w-full" />
...
export function PageHeaderFirstSection({hidden = false}:PageHeaderFirstSectionProps ) {
    ...
    return <div className={`gap-4 items-center flex-shrink-0 
     ${hidden ? "hidden" : "flex"}`}>
    <Button onClick={toggle} variant="ghost" size="icon">
        <Menu />
    </Button>
    <a href="/">
        <img src={Youtube} className="h-12" />
    </a>
</div>
}